...
public class SaveWordBuilder extends QueryBuilder {
    private static final QueryType type = QueryType.SAVE_WORD;
        ...
        List<List<InlineKeyboardButton>> rowList = Collections.singletonList(
                Arrays.asList(
                        createInlineKeyboardButton("Actually, I do!",
                                buildCallbackData(type.getKey(), SaveWordQuery.SaveWordStatus.SAVE.getKey(),
                                        new String[]{wordText})),

                        createInlineKeyboardButton("Not really...",
                                buildCallbackData(type.getKey(), SaveWordQuery.SaveWordStatus.IGNORE.getKey(),
                                        new String[]{wordText}))
                ));
